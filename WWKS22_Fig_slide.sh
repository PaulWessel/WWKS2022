...
gmt begin ${dir}WWKS22_Fig_slide $1
	...
	gmt set FONT_ANNOT_PRIMARY 8p,Times-Italic
	gmt set FONT_LABEL 12p,Times-Italic
	# Top is the model
	gmt plot -R0/2.6/0/1.2 -JX9.35c/3c -W1p -Ba10f1 -BWS --MAP_FRAME_TYPE=graph -Y15c <<- EOF
	0	1
	0.2	1
	1	0
	EOF
	...
	1.05	0.4
	EOF
	gmt text -F+f8p,Times-Italic+j -N -Dj4p <<- EOF
	0.00	0.1	RM	h@-c@-
	...
	1.02	0.4	LM	h@-s@-(r)
	0.72	0.9	LM	h(r)
	2.30	1.1	CB	Map view parameters
	1.90	0.97	LM	@~a@~@-1@-
	2.62	0.78	LM	@~a@~@-2@-
	EOF
	...
	# Next is u0
	gmt basemap -R0/1/0/1 -JX10c/3c -Bxaf+p"u = " -Byaf+l"q(u)" -Y-3.7c
	u0=0.002
	...
	echo "Radial slide shape parameter" | gmt text -F+cTC+f9p -N -Dj4p
	# Last is azimuthal
	gmt basemap -R-1/1/0/1 -JX10c/3c -Bxaf+p"@~g@~ = " -Byaf+l"s(@~g@~)" -Y-3.7c
